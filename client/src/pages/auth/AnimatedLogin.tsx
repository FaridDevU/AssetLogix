...
import { signInWithGoogle, signOut, handleGoogleRedirect } from "@/lib/firebase";
import { motion } from "framer-motion";
import { Loader2, Lock, User, Mail } from "lucide-react";
import IVUMSALogo from "@/assets/ivumsa-logo-recortado.png";
...
export default function AnimatedLogin() {
  ...
  // Ya no usamos redirect con Firebase, ahora usamos popup
  useEffect(() => {
    // Simplemente inicializamos el estado
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // Mostrar el dominio actual para que el usuario lo agregue a Firebase Console
      const currentDomain = window.location.hostname;
      console.log("Dominio que debe estar autorizado en Firebase Console:", currentDomain);
      
      // Intentamos iniciar sesión con Google
      const googleUser = await signInWithGoogle();
      
      // Ya sabemos que googleUser tiene email porque la función signInWithGoogle lo garantiza
      console.log("Usuario de Google autenticado:", googleUser);
      
      try {
        // Ahora necesitamos registrar o iniciar sesión con este usuario en nuestro backend
        // Asumimos que el correo electrónico es el username
        const email = googleUser.email; // Esto es seguro porque GoogleUser garantiza que email es string
        
        // Primero intentamos iniciar sesión 
        try {
          const loginResponse = await loginMutation.mutateAsync({
            username: email,
            password: "google-auth-" + googleUser.uid // Esta contraseña no importa porque no se usará
          });
          
          console.log("Inicio de sesión exitoso con el backend", loginResponse);
          ...
          console.log("El usuario no existe en el sistema, intentando registrar...");
          // Si el inicio de sesión falla, intentamos registrar al usuario
        }
        
        // Si llegamos aquí, el inicio de sesión falló, intentamos registrar
        const registerResponse = await registerMutation.mutateAsync({
          username: email,
          email: email,
          password: "google-auth-" + googleUser.uid, // Esta contraseña nunca se usará para inicios de sesión directos
          name: googleUser.displayName || email.split('@')[0],
          role: 'user'
        });
        
        console.log("Registro exitoso con el backend", registerResponse);
        ...
        toast({
          title: "Error de autenticación con el backend",
          description: "La autenticación con Google fue exitosa, pero no pudimos registrar tu usuario en el sistema.",
          variant: "destructive",
        });
      }
    } catch (error) {
      ...
      // Procesamiento mejorado de errores
      ...
      if (error instanceof Error) {
        ...
        if (errorObj.code === "auth/operation-not-allowed") {
          ...
          helpMessage = "Ve a Firebase Console > Authentication > Sign-in method > Google y actívalo. Debes habilitar el método de inicio de sesión con Google.";
          ...
          helpMessage = `❗️ IMPORTANTE: Agrega '${window.location.hostname}' en Firebase Console > Authentication > Settings > Authorized domains`;
  ...
  useEffect(() => {
    async function checkGoogleRedirect() {
      try {
        ...
        if (user && user.email) {
          // Iniciar sesión en backend usando email+password o registrar al usuario
  ...
  // Función temporal para Apple (solo muestra mensaje)
