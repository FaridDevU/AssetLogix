...
export interface LayoutProps {
  ...
  currentModule: 'dashboard' | 'documentos' | 'inventario' | 'empleados' | 'configuracion' | 'proyectos' | 'admin';
}
...
const Layout: React.FC<LayoutProps> = ({ children, currentModule }) => {
  ...
  const getNavigation = () => {
    const baseNav = [
      ...
      { name: 'Gestión de Empleados', href: '/users', icon: Users, current: currentModule === 'empleados' },
      { name: 'Configuración', href: '/settings', icon: Settings, current: currentModule === 'configuracion' },
    ];
    
    // Añadir opciones de administración solo para administradores
    if (user && user.role === 'admin') {
      baseNav.push(
        { name: 'Administrar Usuarios', href: '/admin/users', icon: Users, current: currentModule === 'admin' }
      );
    }
