...
// Middleware de autenticación exportado para ser usado en otros archivos de rutas
...
// Middleware de verificación de roles exportado para ser usado en otros archivos de rutas
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/documents/:id", authenticate, async (req, res) => {
    try {
      ...
      await storage.logDocumentActivity({
        ...
        userId: user.id,
        action: "view",
        details: `Document ${document.name} viewed`
      });
  ...
  app.get("/api/equipment-types", authenticate, async (req, res) => {
    try {
      console.log("Obteniendo tipos de equipo");
  ...
  // Esta ruta también debe funcionar para mantener compatibilidad
  app.get("/api/equipment/types", authenticate, async (req, res) => {
    try {
      const types = await storage.getEquipmentTypes();
      res.json(types);
  ...
  app.post("/api/equipment-types", authenticate, checkRole(["admin"]), async (req, res) => {
    try {
      ...
      const type = await storage.createEquipmentType(typeData);
      res.status(201).json(type);
    } catch (error) {
      if (error instanceof z.ZodError) {
        res.status(400).json({ message: "Invalid equipment type data", errors: error.errors });
      } else {
        res.status(500).json({ message: "Internal server error" });
      }
    }
  ...
  // Endpoint para eliminar un tipo de equipo
  app.delete("/api/equipment-types/:id", authenticate, checkRole(["admin"]), async (req, res) => {
    try {
      ...
      const equipmentWithType = await storage.getEquipmentByType(id);
      if (equipmentWithType.length > 0) {
        return res.status(400).json({ 
          message: "No se puede eliminar el tipo porque hay equipos que lo utilizan",
          count: equipmentWithType.length
        });
      }
  ...
  app.get("/api/equipment", authenticate, async (req, res) => {
    try {
      console.log("Obteniendo lista de equipo");
      const equipment = await storage.getAllEquipment();
      
      console.log("Equipos obtenidos:", equipment.length);
      
      // Enrich with type information
      const enrichedEquipment = await Promise.all(equipment.map(async (item) => {
        try {
          // Si typeId es null o undefined, no intentamos buscar el tipo
          if (!item.typeId) {
            return {
              ...item,
              type: null
          ...
          const type = await storage.getEquipmentType(item.typeId);
          ...
          console.error(`Error obteniendo tipo para equipo ${item.id}:`, typeError);
          // Si hay error al obtener el tipo, devolvemos el equipo sin tipo
          return {
            ...item,
            type: null
          };
  ...
  app.get("/api/equipment/search", authenticate, async (req, res) => {
    try {
      ...
      const equipment = await storage.searchEquipment(query);
      
      // Enrich with type information
      const enrichedEquipment = await Promise.all(equipment.map(async (item) => {
        const type = item.typeId ? await storage.getEquipmentType(item.typeId) : null;
